...
### 2026-09-17 — One Bedrock model 503'd for hours and the whole fleet hung (^warm-yak)

`us.anthropic.claude-fable-5-1` went ServiceUnavailableException on every route (per-person profile, `us.`, `global.`) from ~14:48 UTC while fable-5 / opus / sonnet answered. Each 503 matched `isTransientApiError` ("bedrock is unable to process"), so every session sat in backoff-nudge purgatory; `looksLikeModelError` never matched, the chain never advanced, and the fleet stayed dead until a human ran `con agent model set us.anthropic.claude-fable-5`. Fix: `UpstreamOutageTracker` counts the 503/529 class fleet-wide per model and, once the failures look like an outage (≥3 over ≥2 min in 10 min, outnumbering successes 2:1), routes through the ordinary `model_failure` → chain-advance path; a 10-min cooldown between advances stops a region-wide outage cascading to haiku. Rule: CLAUDE.md → Transient API-error auto-resume.
