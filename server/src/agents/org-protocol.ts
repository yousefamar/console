...
export function buildBoardProtocol(): string {
  return [
    ...
    '',
    '**Edit boards with `con spaces board`, NOT by hand** — hand-editing markdown is error-prone and the hub serializes concurrent edits for you: `con spaces board <project>` (show), `… move "^id" "Under Review"`, `… add "text" --to Backlog [--assign key]`, `… assign "^id" <key|none>`, `… block "^id" --note "why"` / `… unblock "^id"`, `… note "^id" "text"` (multi-line OK), `… attach "^id" <screenshot.png|clip.webm>`, `… edit "^id" --text "new"`. Cards are addressed by `^id` (safest) or unique text substring. Raw file edits still work as a fallback but risk clobbering concurrent writers.',
    '',
    '- **Being assigned**: a card with `@<yourKey>` under `## In Progress` wakes you with a `[BOARD TASK]` message naming the board file. Do the work, then HAND IT BACK on the board — he reads the CARD, not your transcript, often days later: (1) `con spaces board <p> note "^id" "- …"` with a concise `- ` bulleted summary of EXACTLY what you did (what changed, how verified, anything he must know); (2) `con spaces board <p> attach "^id" <screenshot.png|clip.webm>` (png/jpg/gif/webp, or webm/mp4 ≤20 MB) wherever a visual check helps — REQUIRED when you worked in a worktree or touched UI (he can\'t easily run your worktree; capture your own dev server, never his live tab); (3) only then move the line under `## Under Review`, and end your turn with the same bullets. A move into Under Review with no summary bullets returns a warning — act on it. YOUSEF reviews it and moves it to `## Done`; NEVER move a card to Done yourself, and never approve another agent\'s Under Review card either. When YOUR card reaches Done you get a `[CARD APPROVED — wind down]` message: follow it (merge/clean your worktree), end your turn, and the hub folds your summary into your parent and closes you. Stuck? Append `#blocked` to your line (it keeps its column) with an indented note below explaining what you need; remove the tag when unblocked. Always keep the trailing `^id` token on your line — it is the card\'s identity.',
    '- **Review comments re-open the card**: while your card sits in `## Under Review`, any comment/follow-up Yousef sends you about it means MORE WORK: FIRST — before doing anything else — move the card back to `## In Progress` (`con spaces board <p> move "^id" "In Progress"`), do the work, note a fresh `- ` summary, then move it to `## Under Review` again. The hub appends a `[BOARD — you own a card in Under Review]` reminder to messages you receive while a card of yours is in review; when you see it, the move-back is your first command. The board is his attention queue — Under Review must mean "ready for Yousef", never "being reworked".',
    ...
    '- **Dispatch auto-forks**: when a card is dispatched to a role with a LIVE session, the hub mints a ticket-fork of that session — a FRESH session at the same cwd (it gets CLAUDE.md + auto-memory natively plus a digest of the parent\'s recent conversation in its envelope; the parent\'s whole transcript is NOT copied — that cost ~130k tokens per message), which owns the card (its own `@key` is stamped onto the line) and works the ticket in its own worktree while the main session stays free. A card that genuinely needs the parent\'s full conversation takes a trailing `#inherit` tag (`con spaces board <p> inherit "^id"`; board-wide: frontmatter `fork_context: inherit`) and the fork is spawned as a transcript copy instead. So to parallelize your own work, assign a card to YOURSELF under `## In Progress`: the hub spawns a fork of you per card. No manual forking needed. Trivial cards can OPT OUT with a trailing `#nofork` tag (`con spaces board <p> nofork "^id"`): dispatch wakes the role directly — no fork, no worktree ceremony. A trailing `#model/<alias>` tag — or just `#haiku` / `#sonnet` / `#opus` / `#fable` typed on the card (`con spaces board <p> model "^id" haiku`) — pins the ticket-fork\'s model — cheap/fast cards on haiku, deep ones on opus; ignored on direct wakes.',
    ...
    '- **Worktree discipline**: for code work of any substance, isolate first: `autowt switch <ticket-slug> --terminal echo -y` prints a fresh worktree path (aliased `wt`; no terminal opens). Work THERE — run your own dev server and your own tests inside it. When done, fold the work back into the project\'s main branch per that repo\'s convention (Console: commits land on `main`, branches never survive), then remove YOUR worktree only — `git worktree remove <path> && git branch -d <branch>` (never `autowt cleanup --mode …`, which sweeps every merged worktree including siblings\'; `autowt cleanup <slug> -y` needs a TTY you do not have). You decide per ticket whether it warrants a worktree — trivial doc edits don\'t; anything touching shared running code does.',
