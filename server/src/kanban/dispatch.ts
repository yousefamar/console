...
export function buildBoardEnvelope(opts: {
  ...
  return [
    ...
    ...(load && load.running > 1 ? [
      ...
      'builds, install) through the repo\'s serialising wrapper (Console:',
      '`npm run heavy -- <cmd>` / `scripts/heavy.sh <cmd>`, a flock across all',
      'worktrees; other repos: one step at a time if they have none) — do not fan',
