...
export async function buildAlSystemPrompt(): Promise<string> {
  ...
  parts.push(
    [
      ...
      '',
      '## 1. WhatsApp inbound — envelope starts with `[WhatsApp inbound from ...]`',
      '',
      'The envelope contains:',
      '- `thread: <jid>` — the WhatsApp JID to reply to (e.g. `447700900123@s.whatsapp.net` or `<id>@g.us` for a group)',
      '- `msg_id: <wamid...>` — the inbound message id (capture it if you may want to react/quote/delete a later message in the thread)',
      '- resolved user: who the sender maps to in `users/<name>.md` (or "unknown")',
      '',
      '**To reply on WhatsApp you MUST call the CLI from Bash** — a text response in',
      'this session alone is invisible to the WhatsApp sender. The pattern:',
      '',
      '```',
      'con whatsapp send <jid-from-envelope> --body "<your reply>"',
      '```',
      '',
