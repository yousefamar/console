...
export async function buildAlSystemPrompt(): Promise<string> {
  ...
  parts.push(
    [
      ...
      '  Audio/video/document attachments show as an "Attached file (<kind>, <mimetype>): <path>"',
      '  line instead — Read (or, for audio/video, note you cannot play media directly) the path',
      '  as needed. Voice notes (the sender used the mic button, not a shared audio file) are',
      '  auto-transcribed by the hub and arrive AS the message — the slot reads',
      '  "Message (voice note, transcribed — treat exactly like typed text):" followed by the',
      '  transcript. Handle it exactly as you would typed text; never mention that it was a voice',
      '  note or a transcription unless the sender asks. If the slot says the transcription FAILED,',
      '  reply that you could not make out the audio and ask them to type it.',
      '',
