// ^gold-hare (opsec rem #63): the voice callbacks must pass the hub's auth
// wall, not bypass it. /voice/delegate was an unauthenticated write primitive
// into Al's live session for ~86 days because the exemption list claimed a
// signature check that nothing implemented. The caller is now the local
// voice pipeline (^wise-lark) carrying the `voice` bearer — same wall.
import { describe, it, expect } from 'vitest'
import type { IncomingMessage } from 'node:http'
import { decide, isAlwaysOpenPath } from '../auth-middleware.js'
import type { AuthStore, HubToken } from '../auth-store.js'
import { isSafeCallId, normalisePhone } from '../al/voice.js'
  ...
  return {
    validateHubToken: (plaintext: string): HubToken | null =>
      plaintext === VOICE_TOKEN
        ? ({ id: 'v1', name: 'voice-pipeline', scope: 'voice', hash: 'x', createdAt: 0 } as unknown as HubToken)
        : null,
...
describe('voice callbacks are gated, not exempt', () => {
  it('only /voice/health stays always-open', () => {
    ...
    expect(isAlwaysOpenPath('/voice/transcript', 'POST')).toBe(false)
    expect(isAlwaysOpenPath('/voice/context', 'GET')).toBe(false)
    expect(isAlwaysOpenPath('/voice/call', 'POST')).toBe(false)
  })
  ...
  it('delegate without a bearer is refused (the rem #63 shape, incl. the ?request= GET form)', () => {
    ...
    expect(decide(req({ url: '/voice/transcript', method: 'POST' }), fakeStore()).allow).toBe(false)
    expect(decide(req({ url: '/voice/context?jid=447700900123', method: 'GET' }), fakeStore()).allow).toBe(false)
  })
