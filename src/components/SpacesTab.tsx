// Spaces — the project-first pane. Left rail: areas + projects (a space is a
// projects/ folder or a registry area). Centre: the space's kanban board
// (markdown-backed, hub-dispatched) or its docs (vault notes). Right: a
// narrow agent chat panel — the project's agents, one session at a time.
//
...
// ---------------------------------------------------------------------------
// Docs view — the space's notes, opened in the Notes pane machinery
// ---------------------------------------------------------------------------
...
function DocsView({ space }: { space: SpaceSummary }) {
  ...
  const spaceFiles = useMemo(
    () => files.filter((f) => f.path.startsWith(prefix) || f.path === flat),
    [files, prefix, flat],
  )
  ...
  if (space.kind === 'area') {
    return (
      <div className="flex-1 grid place-items-center text-xs text-text-tertiary px-6 text-center">
        Area writing lives in the blog (tag: {space.slug}) — open the Notes pane for drafts.
      </div>
    )
  }
    ...
    <div className="flex-1 overflow-y-auto p-2">
      {spaceFiles.length === 0 && (
        <div className="px-2 py-4 text-xs text-text-tertiary">
          No files under {prefix} — open the Notes pane to reconnect the vault, or create the folder.
        </div>
      )}
      {spaceFiles.map((f) => (
        <button
          key={f.path}
          onClick={() => void openInNotes(f.path)}
          className="flex w-full items-center gap-2 rounded-sm px-2 py-1 text-left text-xs text-text-secondary hover:bg-surface-1 hover:text-text-primary"
        >
          <FileText size={10} className="flex-shrink-0 opacity-50" />
          <span className="truncate">{f.path.slice(f.path.startsWith(prefix) ? prefix.length : 'projects/'.length)}</span>
        </button>
      ))}
    </div>
  )
...
// ---------------------------------------------------------------------------
// Right rail — the space's agents + the active session, narrow
// ---------------------------------------------------------------------------
...
function SpaceAgentPanel({ space }: { space: SpaceSummary }) {
  ...
  const activeBelongs = spaceRoles.some((r) => sessionFor(r.key)?.id === activeSessionId)
      ...
      <div className="flex flex-shrink-0 items-center gap-1.5 border-b border-border px-2 py-1.5 overflow-x-auto">
        ...
        {spaceRoles.length === 0 && (
          <span className="text-[10px] text-text-tertiary">
            No agents claim this space (role frontmatter: {space.kind === 'project' ? `project: ${space.slug}` : `areas: [${space.slug}]`})
          </span>
        )}
        {spaceRoles.map((r) => {
        ...
        <button onClick={() => setCollapsed(true)} className="ml-auto flex-shrink-0 text-text-tertiary hover:text-text-primary" title="Collapse">
          <X size={11} />
        </button>
      </div>
      <div className="flex-1 min-h-0 flex flex-col">
        {activeBelongs ? (
          ...
          <div className="flex-1 grid place-items-center px-4 text-center text-[11px] text-text-tertiary">
            {spaceRoles.length ? 'Pick an agent above' : 'Stamp a role file to bind an agent here'}
          </div>
        )}
      </div>
    </div>
