...
- **Hub server** — Node.js backend (`server/`) proxies all API calls (Gmail, Calendar, Matrix, Monzo), manages OAuth tokens, hosts agent sessions. Internal HTTPS via any `*.crt`/`*.key` pair in `~/.config/console/` (Caddy terminates the public TLS in front). Binds `0.0.0.0:9877` by default; Phase 9 will lock to `127.0.0.1`.
