...
- **Notes → glasses mirror** (`src/glasses/notes-mirror.ts`): toggle in Glasses settings. Follows the cursor — last row on glasses is the line *below* the cursor (or the cursor line itself at EOF); cursor sits on row 4. Renders **line numbers + a `|` cursor glyph** at the cursor's column, **word-wrapped at 40 cols** (matches EvenDemoApp / even-web reference). Hook is in `NotesEditorCore`'s `updateListener`; writes go straight to `ConsoleNative.glassesSendText` (no hub round-trip). 30 ms debounce, coalesces stale keystrokes. Persisted via localStorage (`console:glasses:notesMirrorEnabled`).
- **Notes-mirror "stealth screen"** (APK v12+): turning the mirror toggle on calls `ConsoleNative.setNotesMirrorDim(true)` which holds `FLAG_KEEP_SCREEN_ON` and sets `screenBrightness = 0.01f`. The panel looks off but the Activity stays foreground so HW-keyboard keystrokes keep reaching the WebView. Re-applied on boot in `wireGlassesStore()` when the persisted toggle is on.
