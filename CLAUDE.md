...
- **UI** (`src/components/AgentTab.tsx`): a view-mode toggle (`agentViewMode`, localStorage `console:agents:viewMode`) flips the Agents pane between the session **list** and a visual **org chart** — `src/components/agent/AgentOrgChart.tsx`, a canvas node-link **tree** (`d3-hierarchy` `tree()` + `d3-zoom`, reusing the Notes circles-view canvas/zoom/drag engine; pure layout/hit-test in `agent-orgchart-helpers.ts`, unit-tested). Click a node → open it; drag a node onto another → reparent (drop on empty space → make it a root). `AgentProfilePanel.tsx` shows charter/goals/`## Memory` (read-only — agent-owned) + a manager dropdown + revive/reload/park/delete actions.
- **Out of scope** (deliberately not paperclip): no DB/ticket-tracker/budgets. Per-agent heartbeats + structured manager→report delegation are a deferred follow-up (reuse the existing cron + fork/chat).
